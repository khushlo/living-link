 ...
 * and ONC/HHS de-identified population analytics
 */
...
// Simple de-identification: remove name, DOB, phone. Keep gender + ID.
function deidentifyPatient(patient: ReturnType<typeof mapDonorToFHIRPatient>) {
...
export async function POST(req: NextRequest) {
  ...
  // Require ADMIN or CLINICIAN role
  const user = await prisma.user.findUnique({ where: { clerkId: userId! } });
  if (!user || !["ADMIN", "CLINICIAN", "COORDINATOR"].includes(user.role)) {
  ...
  try {
    ...
    for (const profile of profiles) {
      ...
      patientResources.push(deidentifyPatient(patient));
      ...
      for (const checkin of profile.checkins) {
        if (checkin.bpSystolic && checkin.bpDiastolic) {
          observationResources.push(
            mapBPToFHIRObservation(profile.id, checkin.bpSystolic, checkin.bpDiastolic, checkin.completedAt)
          );
        }
        if (checkin.weightKg) {
          observationResources.push({
      ...
      for (const phq2 of profile.phq2Responses) {
        qrResources.push(
          mapPHQ2ToFHIRQuestionnaireResponse(
            profile.id, phq2.id, phq2.q1Score, phq2.q2Score, phq2.completedAt
          )
        );
      }
        ...
        operation: [
          {
            ...
            documentation: "Export de-identified LifeAfter outcomes for OPTN Policy 18 and ONC reporting.",
          },
        ],
      },
    ],
